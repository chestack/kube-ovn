...
func (c *Controller) handleFip(op string, podName string, as map[string]string) error {
	...
	if _, ok := as[util.EipAnnotation]; ok {
		eip = as[util.EipAnnotation]
	}
	if _, ok := as[util.SnatAnnotation]; ok {
		snat = as[util.SnatAnnotation]
	}
	if eip == "" && snat == "" {
	...
	if _, ok := as[util.ProjectIDAnnotation]; ok {
		projectID = as[util.ProjectIDAnnotation]
	}
	if projectID == "" {
	...
	if _, ok := as[util.LogicalRouterAnnotation]; ok {
		logicalRouter = as[util.LogicalRouterAnnotation]
	}
	if logicalRouter == "" {
	...
	case "add":
		// 判断eip是否已经被其他pod占用
		if isOtherPodEipAllocated(eip, podName, oldFip) {
		...
		if isAvailable(eip, oldFip) {
			...
			err = neutron.NewClient().AddTag("floatingips", floatingip.ID, util.NeutronRouterTag)
			if err != nil {
			...
			fipPatch := genEipPatch(op, eip, podName, oldFip)
			klog.Infof("add FipPatch to syncFipQueue, FipPatch: %+v\n", fipPatch)
			c.neutronController.syncFipQueue.Add(fipPatch)
		}
		...
		if snat != "" {
			...
			// 这里需要修改 fip cr status，生成对应的 fip cr patch 内容
			fipPatch := genSnatPatch(op, snat, podName, oldFip)
			klog.Infof("add FipPatch to syncFipQueue, FipPatch: %+v\n", fipPatch)
			c.neutronController.syncFipQueue.Add(fipPatch)
		}
	case "del":
		// 判断eip是否正在被当前pod使用
		if isPodEipAllocated(eip, podName, oldFip) {
		...
		// 判断snat是否正在被当前pod使用
		if isSnatAllocated(snat, podName, oldFip) {
			// 判断snat是否正在被当前pod唯一使用
			// 若snat被当前pod唯一使用，则pod删除后需要同步删除对应proton floatingip资源
			if isSnatUniqueAllocated(snat, podName, oldFip) {
				// 删除 proton floatingip 资源
