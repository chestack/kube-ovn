...
type PortLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Port, err error)
...
// List lists all Ports in the indexer.
func (s *portLister) List(selector labels.Selector) (ret []*v1.Port, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Port))
	})
	return ret, err
}
...
// List lists all Ports in the indexer for a given namespace.
func (s portNamespaceLister) List(selector labels.Selector) (ret []*v1.Port, err error) {
...
// Get retrieves the Port from the indexer for a given namespace and name.
func (s portNamespaceLister) Get(name string) (*v1.Port, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("port"), name)
	}
	return obj.(*v1.Port), nil
}
